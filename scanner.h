...
enum Instructions { load, loadI, store, add, sub, mult, lshift, rshift,output,nop};
enum InstructionCategory { MEMOP, LOADI, ARITHOP, NOP, OUTPUT, ERROR };
enum TokenCategory { INST, CONST, ARROW, REG, COMMA, COMMENT, DIGIT };
...
class Inst {
    ...
    Op op2;
    Op dest;
    Inst(int line, OpInst opcode, int o1, int o2, int d);
    void tablePrint();
